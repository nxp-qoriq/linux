...
static void enetc4_set_trx_frame_size(struct enetc_pf *pf)
{
	...
	ecat_enetc_port_mac_wr(si, ENETC4_PM_MAXFRM(0),
...
static void enetc4_set_default_rss_key(struct enetc_hw *hw)
{
	...
	ecat_enetc_set_rss_key(hw, hash_key);
}
...
static void enetc4_pf_set_mac_hash_filter(struct enetc_pf *pf, int type)
{
	...
	if (type & ENETC_MAC_FILTER_TYPE_UC) {
		...
		ecat_enetc_reset_mac_addr_filter(mac_filter);
		netdev_for_each_uc_addr(ha, ndev)
			ecat_enetc_add_mac_addr_ht_filter(mac_filter, ha->addr);
...
static const struct net_device_ops enetc4_ndev_ops = {
	...
	.ndo_eth_ioctl		= ecat_enetc_enetc_ioctl,
	.ndo_setup_tc		= enetc_pf_setup_tc,
	.ndo_bpf		= ecat_enetc_setup_bpf,
	.ndo_xdp_xmit		= ecat_enetc_xdp_xmit,
	.ndo_xsk_wakeup		= ecat_enetc_xsk_wakeup,
};
			      ...
			      phy_interface_t phy_mode)
{
	...
	val = ecat_enetc_port_mac_rd(si, ENETC4_PM_IF_MODE(0));
	...
	ecat_enetc_port_mac_wr(si, ENETC4_PM_IF_MODE(0), val);
}
...
static void enetc4_set_rgmii_mac(struct enetc_pf *pf, int speed, int duplex)
{
	...
	old_val = ecat_enetc_port_mac_rd(si, ENETC4_PM_IF_MODE(0));
...
static void enetc4_set_hd_flow_control(struct enetc_pf *pf, bool enable)
{
	...
	old_val = ecat_enetc_port_mac_rd(si, ENETC4_PM_CMD_CFG(0));
	val = u32_replace_bits(old_val, enable ? 1 : 0, PM_CMD_CFG_HD_FCEN);
	if (val == old_val)
		return;

	ecat_enetc_port_mac_wr(si, ENETC4_PM_CMD_CFG(0), val);
}
...
static void enetc4_set_tx_pause(struct enetc_pf *pf, int num_rxbdr, bool tx_pause)
{
	...
	ecat_enetc_port_mac_wr(pf->si, ENETC4_PM_PAUSE_QUANTA(0), init_quanta);
	ecat_enetc_port_mac_wr(pf->si, ENETC4_PM_PAUSE_THRESH(0), refresh_quanta);
...
static void enetc4_enable_mac(struct enetc_pf *pf, bool en)
{
	...
	val = ecat_enetc_port_mac_rd(si, ENETC4_PM_CMD_CFG(0));
				  ...
				  int duplex, bool tx_pause, bool rx_pause)
{
	...
	ecat_enetc_eee_mode_set(si->ndev, priv->eee.eee_active);

	if (si->hw_features & ENETC_SI_F_QBU)
		ecat_enetc_mm_link_state_update(priv, true);
				    ...
				    phy_interface_t interface)
{
	...
	if (si->hw_features & ENETC_SI_F_QBU)
		ecat_enetc_mm_link_state_update(priv, false);
...
static int enetc4_pf_netdev_create(struct enetc_si *si)
{
	...
	ecat_enetc_init_si_rings_params(priv);
	err = ecat_enetc_configure_si(priv);
	if (err) {
	...
	err = ecat_enetc_alloc_msix(priv);
	if (err) {
			   ...
			   const struct pci_device_id *ent)
{
	...
	err = ecat_enetc_pci_probe(pdev, KBUILD_MODNAME, sizeof(*pf));
	if (err) {
...
err_pf_struct_init:
err_ecat_enetc_pci_probe:
	ecat_enetc_pci_remove(pdev);
...
{
	u32 val = ecat_enetc_port_mac_rd(si, ENETC4_PM_CMD_CFG(0));
	...
	ecat_enetc_port_mac_wr(si, ENETC4_PLPMR, en ? PLPMR_WME : 0);
}
...
static int __maybe_unused enetc4_pf_suspend(struct device *dev)
{
	...
	rtnl_lock();
	ecat_enetc_suspend(si->ndev, netc_ierb_may_wakeonlan() > 0);
...
static int __maybe_unused enetc4_pf_resume(struct device *dev)
{
	...
	ecat_enetc_resume(si->ndev, netc_ierb_may_wakeonlan() > 0);
