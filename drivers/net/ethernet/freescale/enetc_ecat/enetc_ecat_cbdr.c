...
EXPORT_SYMBOL_GPL(ecat_enetc_send_cmd);
...
int ecat_enetc_clear_mac_flt_entry(struct enetc_si *si, int index)
{
	...
	return ecat_enetc_send_cmd(si, &cbd);
}
...
EXPORT_SYMBOL_GPL(ecat_enetc_set_mac_flt_entry);
		       ...
		       int index)
{
	...
	err = ecat_enetc_send_cmd(si, &cbd);
	if (err)
...
EXPORT_SYMBOL_GPL(ecat_enetc_get_rss_table);
