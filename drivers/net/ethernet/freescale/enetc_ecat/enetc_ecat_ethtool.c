...
l4ip4:
	...
	return ecat_enetc_set_fs_entry(si, &rfse, fs->location);
}
...
EXPORT_SYMBOL_GPL(ecat_enetc_set_rss_key);
			  ...
			  struct netlink_ext_ack *extack)
{
	...
	/* set hash key, if PF */
	if (rxfh->key && enetc_si_is_pf(si))
		ecat_enetc_set_rss_key(hw, rxfh->key);
...
void ecat_enetc_eee_mode_set(struct net_device *dev, bool enable)
{
	...
	ecat_enetc_port_mac_wr(si, ENETC4_PM_SLEEP_TIMER(0), sleep_cycle);
	ecat_enetc_port_mac_wr(si, ENETC4_PM_LPWAKE_TIMER(0), wake_cycle);
}
EXPORT_SYMBOL_GPL(ecat_enetc_eee_mode_set);
...
EXPORT_SYMBOL_GPL(ecat_enetc_mm_link_state_update);
...
EXPORT_SYMBOL_GPL(ecat_enetc_set_ethtool_ops);
