...
int enetc_vlan_rx_add_vid(struct net_device *ndev, __be16 prot, u16 vid)
{
	...
	idx = ecat_enetc_vid_hash_idx(vid);
...
int enetc_vlan_rx_del_vid(struct net_device *ndev, __be16 prot, u16 vid)
{
	...
	if (__test_and_clear_bit(vid, si->active_vlans)) {
		idx = ecat_enetc_vid_hash_idx(vid);
		ecat_enetc_refresh_vlan_ht_filter(si);
...
int enetc_pf_set_features(struct net_device *ndev, netdev_features_t features)
{
	...
	ecat_enetc_set_features(ndev, features);
		      ...
		      void *type_data)
{
	...
	case TC_SETUP_QDISC_MQPRIO:
		return ecat_enetc_setup_tc_mqprio(ndev, type_data);
			   ...
			   const struct net_device_ops *ndev_ops)
{
	...
	ecat_enetc_set_ethtool_ops(ndev);
