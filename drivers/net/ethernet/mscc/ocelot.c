...
int ocelot_init(struct ocelot *ocelot)
{
	...
	if (ocelot->ptp) {
		ret = ocelot_init_timestamp(ocelot, &ocelot_ptp_clock_info);
		if (ret) {
			dev_err(ocelot->dev,
		...
		ptp_rule = kzalloc(sizeof(*ptp_rule), GFP_KERNEL);
		if (!ptp_rule)
			return -ENOMEM;
		 ...
		 */
		ptp_rule->prio = 1;
		ptp_rule->type = OCELOT_ACE_TYPE_ETYPE;
		ptp_rule->dmac_mc = OCELOT_VCAP_BIT_1;
		ptp_rule->action = OCELOT_ACL_ACTION_TRAP;
		ptp_rule->frame.etype.etype.value[0] = 0x88;
		ptp_rule->frame.etype.etype.value[1] = 0xf7;
