			  ...
			  u64 phys, u64 bus_addr, u8 func, u64 size)
{
	...
	if (phys != pcie->ep.phys_base)
		size = ALIGN(size, phys);
				     ...
				     u8 func_no, u8 bar)
{
	...
	val = csr_readl(pci, PAB_PEX_BAR_AMAP(func_no, bar));
	val &= ~(1 << 0);
	csr_writel(pci, val, PAB_PEX_BAR_AMAP(func_no, bar));
}
...
void mobiveil_pcie_enable_bridge_pio(struct mobiveil_pcie *pci)
{
	...
	val = csr_readl(pci, PAB_CTRL);
	val |= 1 << AMBA_PIO_ENABLE_SHIFT;
	val |= 1 << PEX_PIO_ENABLE_SHIFT;
	csr_writel(pci, val, PAB_CTRL);
}
...
void mobiveil_pcie_enable_engine_apio(struct mobiveil_pcie *pci)
{
	...
	val = csr_readl(pci, PAB_AXI_PIO_CTRL);
	val |= APIO_EN_MASK;
	csr_writel(pci, val, PAB_AXI_PIO_CTRL);
}
...
void mobiveil_pcie_enable_engine_ppio(struct mobiveil_pcie *pci)
{
	...
	val = csr_readl(pci, PAB_PEX_PIO_CTRL);
	val |= 1 << PIO_ENABLE_SHIFT;
	csr_writel(pci, val, PAB_PEX_PIO_CTRL);
}
...
void mobiveil_pcie_enable_msi_ep(struct mobiveil_pcie *pci)
{
	...
	val =  csr_readl(pci, PAB_INTP_AMBA_MISC_ENB);
	val |= 1 << 0;
	csr_writel(pci, val, PAB_INTP_AMBA_MISC_ENB);
}
