...
static int fspi_readl_poll_tout(struct nxp_fspi *f, void __iomem *base, u32 reg,
				 ...
				 const struct spi_mem_op *op)
{
	...
	for (i = 0; i < size; i++) {
		...
		ret = fspi_readl_poll_tout(f, f->iobase + FSPI_INTR, reg,
					   FSPI_INTR_IPTXWE, 0, POLL_TOUT_US);
		WARN_ON(ret);
...
static int nxp_fspi_default_setup(struct nxp_fspi *f)
{
	...
	/* enable module */
	f->write(FSPI_MCR0_AHB_TIMEOUT(0xFF) | FSPI_MCR0_IP_TIMEOUT(0xFF),
		 base + FSPI_MCR0);
...
static int nxp_fspi_probe(struct platform_device *pdev)
{
	...
	if (f->is_acpi_node) {
		res = platform_get_resource(pdev, IORESOURCE_MEM, FSPI_BASE);
	} else {
	...
	if (f->is_acpi_node)
		res = platform_get_resource(pdev, IORESOURCE_MEM, FSPI_MMAP);
	else
		res = platform_get_resource_byname(pdev, IORESOURCE_MEM,
								"fspi_mmap");
